...
async function createSession() {
    ...
    let rawPhone = document.getElementById('phoneInput').value.trim();
    ...
    if (token.length < 6) {
        showToast('❌ Token terlalu pendek (minimal 6 karakter)', 'error');
        return;
    }
    ...
    if (sessions.find(s => s.phone === phone)) {
        showToast('❌ Nomor ini sudah punya session aktif', 'error');
        return;
    }
    
    // ========== CEK STATUS SERVER (MAINTENANCE MODE) ==========
    try {
        const serverCheck = await fetch('api/pterodactyl-status.php');
        const serverData = await serverCheck.json();
        if (!serverData.is_online) {
            showToast('⚠️ GANGGUAN LAYANAN — Server bot sedang offline. Fitur pembuatan session baru akan aktif kembali setelah server normal :)', 'error');
            return;
        }
    } catch (serverErr) {
        console.warn('Gagal cek status server:', serverErr);
        // Lanjutkan meskipun gagal cek server (anggap online)
    }
    
    // ========== PROSES CREATE SESSION ==========
    const btn = document.getElementById('createBtn');
    const originalText = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i> Memeriksa token...';
    
    addLog('info', `Membuat session baru untuk +${phone}`);
    addLog('debug', `Token: ${token}, Script: ${script}, Fingerprint: ${fingerprint.substring(0, 10)}...`);
    
    try {
        // 1. Verifikasi token
        addLog('debug', `Memeriksa token: ${token}`);
        ...
        const tokenData = redeemData[0];
        addLog('debug', `Token data: used=${tokenData.used}, created_at=${tokenData.created_at}`);
        
        if (tokenData.used) {
            throw new Error('Token sudah pernah digunakan sebelumnya');
        }
        ...
        if (tokenAge > maxAge) {
            const expiredMinutes = Math.floor(tokenAge / 60000);
            throw new Error(`Token sudah expired (${expiredMinutes} menit yang lalu). Token hanya berlaku 10 menit.`);
        }
        
        addLog('success', 'Token valid, memproses pembuatan session...');
        btn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i> Membuat session...';
        
        // 2. Update token status (tandai sudah digunakan)
        ...
        // 3. Buat session baru di tabel polar_sessions
        const newSession = {
    ...
    token_used: token,
    pairing_code: null,
    created_at: Date.now()  // ← Kirim angka (Unix timestamp milliseconds)
};
        
        addLog('debug', `Mengirim data session: fingerprint=${fingerprint.substring(0,10)}..., phone=${phone}, script=${script}`);
        
        const result = await supabaseRequest('POST', 'polar_sessions', newSession);
        
        addLog('success', `Session +${phone} berhasil dibuat`);
        showToast('✅ Session berhasil dibuat! Menunggu pairing code...', 'success');
        
        // Tutup modal dan refresh data
        closeAddModal();
        await loadSessions();
        
        // Buka modal pairing setelah session terlihat
        setTimeout(() => {
            const newSessionData = sessions.find(s => s.phone === phone);
            if (newSessionData) {
                openPairModal(phone);
            } else {
                addLog('warning', 'Session baru belum terlihat, silakan refresh manual');
                showToast('Session dibuat, tunggu sebentar lalu refresh halaman', 'info');
            }
        }, 2000);
        
    } catch (error) {
        addLog('error', `Gagal membuat session: ${error.message}`);
